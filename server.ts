...
async function callNvidiaApi(
  ...
  // Moonshot Kimi K3 is the verified ultra-responsive chat model on NVIDIA NIM (build.nvidia.com)
  modelsToTry.push('moonshotai/kimi-k3');

  if (requestedModel && requestedModel !== 'moonshotai/kimi-k3' && requestedModel !== 'deepseek-ai/deepseek-v4-pro-0813') {
    modelsToTry.unshift(requestedModel);
  }

  // DeepSeek is kept as secondary due to frequent high-queue latency on NVIDIA servers
  if (requestedModel === 'deepseek-ai/deepseek-v4-pro-0813') {
    modelsToTry.push('deepseek-ai/deepseek-v4-pro-0813');
  }
  ...
  for (const model of uniqueModels) {
    try {
      ...
      if (!response.ok) {
        ...
        console.warn(`[AI Gateway] NVIDIA model ${model} returned (${response.status}):`, errorText.slice(0, 150));
      ...
      console.warn(`[AI Gateway] NVIDIA model ${model} warning: ${err?.message || err}`);
      lastError = err;
    }
  }

  if (lastError) {
    console.warn('[AI Gateway] NVIDIA models exhausted or timed out. Passing smoothly to cloud backup...');
  }
  return null;
}
...
async function callGeminiApi(
  ...
  console.log('[AI Gateway] Invoking Google Gemini API (gemini-3.6-flash)...');
  ...
  if (nvidiaKey && nvidiaKey !== 'MY_NVIDIA_API_KEY') {
    ...
    try {
      ...
      if (nvidiaResult && nvidiaResult.text) {
        ...
        fallbackFrom = 'nvidia';
        fallbackReason = 'NVIDIA NIM servisi geçici olarak meşgul veya kota limitine ulaştı, otomatik yedek devreye girdi';
      }
    } catch (err: any) {
...
app.get('/api/ai/config', (req, res) => {
  ...
  const supportedNvidiaModels = [
    {
      ...
      category: 'Hızlı & Yüksek Doğruluklu Zeka (Aktif & Doğrulandı)',
      status: 'active',
      tested: true,
    },
    {
      ...
      category: 'Ultra Hızlı Çok Dilli Çeviri (230ms)',
      status: 'active',
      tested: true,
    },
    {
      ...
      status: 'queued',
      tested: false,
      note: 'NVIDIA sunucularındaki yüksek kuyruk nedeniyle Kimi K3 önceliklidir',
    },
    {
  ...
  res.json({
    ...
    defaultNvidiaModel: 'moonshotai/kimi-k3',
...
app.post('/api/ai/test', async (req, res) => {
  ...
  try {
    if (provider === 'nvidia') {
      const resData = await callNvidiaApi(testMessages, sysInst, 'fast', model || 'moonshotai/kimi-k3');
      ...
      if (resData) {
        ...
        return res.status(504).json({
          ...
          status: 'error',
          responsePreview: '',
          error: 'NVIDIA NIM sunucusu zaman aşımına uğradı veya yanıt vermedi. Sistem otomatik olarak Groq/Gemini yedek katmanına yönlendirecektir.',
          timestamp: new Date().toISOString(),
        });
      }
    }
